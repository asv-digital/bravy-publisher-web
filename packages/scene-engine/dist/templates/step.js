...
function corners(nodes, prefix, color) {
    const inset = 32;
    const len = 36;
    const th = 1.5;
    const z = 1;
...
function bgNum(nodes, ctx, prefix, page) {
    const { tokens, metrics } = ctx;
...
function headlineStyleOf(tokens, size, inkTok) {
    return (k) => {
        switch (k) {
...
function bodyStyleOf(tokens, size = 34) {
    return (k) => {
        switch (k) {
...
// ---------------- BODY ----------------
function buildBody(slide, sourceIndex, page, totalPages, ctx) {
    const { tokens, metrics } = ctx;
...
// ---------------- COVER ----------------
function buildCover(content, totalPages, ctx) {
    const { tokens, metrics } = ctx;
...
// ---------------- CTA ----------------
function buildCta(content, totalPages, ctx) {
    const { tokens, metrics } = ctx;
    ...
    const ctaStyleOf = (k) => {
        ...
        if (k === 'keyword' || k === 'code')
            return st(tokens, 'mono', 500, 62 * 0.78, 'bg', { lh: 1.08 });

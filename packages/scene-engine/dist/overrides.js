...
export function validateOverride(ov, _palette) {
    const out = {};
    if (ov.hidden != null)
        out.hidden = ov.hidden;
    if (ov.rotation != null && Number.isFinite(ov.rotation))
        out.rotation = ov.rotation;
    if (ov.fontScale != null && Number.isFinite(ov.fontScale))
        out.fontScale = clamp(ov.fontScale, 0.25, 4);
    if (ov.opacity != null && Number.isFinite(ov.opacity))
        out.opacity = clamp(ov.opacity, 0.05, 1);
    // cor livre (mini-Figma): qualquer hex válido — paleta vira sugestão, não trava
    if (ov.fill != null && /^#[0-9a-fA-F]{3,8}$/.test(ov.fill))
        out.fill = ov.fill;
    if (ov.family)
        out.family = ov.family;
    if (ov.weight != null && Number.isFinite(ov.weight))
        out.weight = ov.weight;
    if (ov.frame)
        out.frame = ov.frame; // clamp depende do frame base → no apply
    return out;
}
function clampFrame(base, delta) {
...
function textOrigins(nodes) {
    const o = new Map();
    for (const n of nodes) {
        if (n.type !== 'glyphrun' || !n.container)
            continue;
        const cur = o.get(n.container);
        if (!cur)
            o.set(n.container, { x: n.x, b: n.baselineY });
        else
            o.set(n.container, { x: Math.min(cur.x, n.x), b: Math.min(cur.b, n.baselineY) });
    }
    return o;
}
/** bbox por âncora de override (id ou container) — p/ âncora de rotação. */
function anchorBounds(nodes, metrics) {
    const out = new Map();
    const grow = (key, x0, y0, x1, y1) => {
        const c = out.get(key);
        if (!c)
            out.set(key, { x0, y0, x1, y1 });
        else
            out.set(key, { x0: Math.min(c.x0, x0), y0: Math.min(c.y0, y0), x1: Math.max(c.x1, x1), y1: Math.max(c.y1, y1) });
    };
    for (const n of nodes) {
 ...
 */
export function applyOverrides(nodes, ov, palette, metrics) {
    ...
    const isDescendantHidden = (n) => hiddenAnchors.some((a) => n.id.startsWith(`${a}.`) || (n.container != null && n.container !== a && n.container.startsWith(`${a}.`)));
    ...
    return nodes.map((n) => {
        ...
        if (!raw && !cascadeHidden && !cascadeMove)
            return n;
        ...
        if (o.hidden || cascadeHidden)
            m.opacity = 0;
        else if (o.opacity != null)
            m.opacity = o.opacity * (n.opacity ?? 1);
        if (o.fill) {
            if (m.type === 'glyphrun')
                m.style = { ...m.style, fill: o.fill };
            else if (m.type === 'rect')
                m.fill = o.fill;
        }
        if (m.type === 'glyphrun') {
 ...
 */
export function reconcileOverrides(prev, currentIds) {
...
export function anchorableIds(nodes) {
    const ids = new Set();
    for (const n of nodes) {

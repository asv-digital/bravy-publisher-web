...
const Z_BASE = 1000;
function textStyle(node, tokens) {
...
function userStyleOf(node, tokens) {
...
export function materializeUserNodes(added, tokens, metrics) {
    if (!added?.length)
        return [];
    const out = [];
    added.forEach((n, i) => {
        ...
        const rc = rot != null && n.kind !== 'line'
            ? { x: n.frame.x + n.frame.w / 2, y: n.frame.y + n.frame.h / 2 }
            : undefined;
        switch (n.kind) {
            case 'text': {
                const block = layoutBlock({ runs: parseInline(n.text), width: Math.max(24, n.frame.w), styleOf: userStyleOf(n, tokens), align: n.align }, metrics);
...
export function measureUserText(node, tokens, metrics) {
    return layoutBlock({ runs: parseInline(node.text), width: Math.max(24, node.frame.w), styleOf: userStyleOf(node, tokens), align: node.align }, metrics).height;
}

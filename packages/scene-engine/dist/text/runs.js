...
export function parseInline(input, base = 'ink') {
    const runs = [];
    const stack = [{ key: base }];
    let i = 0;
    let buf = '';
    ...
    const flush = () => {
        if (buf) {
            const f = cur();
            const run = { text: decodeEntities(buf), key: f.key };
            if (f.underline)
                run.underline = true;
            if (f.color)
                run.color = f.color;
...
export function headlineRuns(top, em, bottom) {
    const runs = [];
    if (top?.trim())
        runs.push({ text: top.trim() + (em || bottom ? ' ' : ''), key: 'ink' });
    if (em?.trim())
        runs.push({ text: em.trim() + (bottom ? ' ' : ''), key: 'em' });
    if (bottom?.trim())
        runs.push({ text: bottom.trim(), key: 'ink' });

...
import { parseInline, type StyleKey, type StyledRun } from '../text/runs.js';
...
function centerGlyph(nodes: SceneNode[], id: string, text: string, style: ResolvedTextStyle, cx: number, baselineY: number, metrics: BuildCtx['metrics'], z: number, opacity?: number): void {
...
function rowsFrom(slide: SlideText): StyledRun[][] {
  if (slide.list) return slide.list.map((i) => parseInline(i));
  if (slide.paragraphs) return slide.paragraphs.map((p) => parseInline(p));
  if (slide.stats) return slide.stats.map(([n, t]) => [{ text: n, key: 'em' as StyleKey }, { text: ` — ${t}`, key: 'ink' as StyleKey }]);
  if (slide.cards) return slide.cards.map((c) => [{ text: c.title ?? '', key: 'strong' as StyleKey }, { text: ` — ${c.body ?? ''}`, key: 'ink' as StyleKey }]);
  return [];
}
...
export const compendiumTemplate: TemplateProgram = {
  family: 'compendium',
  build(content, ctx) {
    Z = 0;
    void Z;
